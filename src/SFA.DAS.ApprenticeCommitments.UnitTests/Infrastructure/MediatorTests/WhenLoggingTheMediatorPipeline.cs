...
namespace SFA.DAS.ApprenticeCommitments.UnitTests.Infrastructure.MediatorTests
{
    public class WhenLoggingTheMediatorPipeline
    {
        ...
        {
            var response = await _sut.Handle(request, CancellationToken.None, _ => Task.FromResult(expectedResponse));
        ...
        {
            await _sut.Handle(request, CancellationToken.None, _ => Task.FromResult(expectedResponse));
        ...
        {
            Func<Task> action = () => _sut.Handle(request, CancellationToken.None, _ => throw new Exception("failed"));
